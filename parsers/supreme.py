...
def parse(url: str):
    ...
    if not title:
        ...
        if meta_title and isinstance(meta_title, Tag) and meta_title.has_attr("content"):
            title = meta_title["content"]
    if not title:
        title = soup.title.string.strip() if soup.title and soup.title.string else None
    if not title:
        print("[Supreme] Не найдено название! Фрагмент:", soup.prettify()[:2000])
    ...
    if not price:
        ...
        if meta_price and isinstance(meta_price, Tag) and meta_price.has_attr("content"):
            price = str(meta_price["content"]) + " $"
    if not price:
        print("[Supreme] Не найдена цена! Фрагмент:", soup.prettify()[:2000])
        price = "Не удалось найти цену"
    # Фото
    img_url = None
    og_img = soup.find("meta", property="og:image")
    if og_img and isinstance(og_img, Tag) and og_img.has_attr("content"):
