...
def parse(url: str):
    ...
    if not title:
        print("[Farfetch] Не найдено название! Фрагмент:", soup.prettify()[:2000])
        title = "Не удалось найти название"
    # Цена
    price = None
    price_tag = soup.find(lambda tag: tag.name in ["span", "div"] and ("€" in tag.get_text() or "$" in tag.get_text()))
    if price_tag:
    ...
    if not price:
        print("[Farfetch] Не найдена цена! Фрагмент:", soup.prettify()[:2000])
